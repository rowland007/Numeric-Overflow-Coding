/************************************************************************
 ...
 Date:          2021-10-25
 Description:   Assignment presents code that is designed to show the two
 functions operating with and without overflow or underflows Input: Output:
 Known bugs:
 Missing features:
 ...
 ************************************************************************/
// NumericOverflows.cpp : This file contains the 'main' function. Program
    ...
    try {
      *temp = result + increment;
      if (result > 0 && increment > 0 &&
          *temp < 0) // Detects overflow of negative result with the sum of two
                     // positive arguments
      {
        throw std::overflow_error("Overflow detected, last good value: ");
      }
      if (result < 0 && increment < 0 &&
          *temp > 0) // Detects overflow of positive result with the sume of two
                     // negative arguments
      {
        throw std::overflow_error("Overflow detected, last good value: ");
      }
...
template <typename T>
T subtract_numbers(T const &start, T const &decrement,
                   unsigned long int const &steps) {
  T result = start;
  T *temp = new T;
  ...
  for (unsigned long int i = 0; i < steps; ++i) {
    try {
      *temp = result - decrement;
      if (result > 0 && decrement > 0 &&
          *temp < 0) // Detects underflow of negative result with two positive
                     // arguments
      {
        throw std::underflow_error("Underflow detected, last good value: ");
      }
      if (result < 0 && decrement < 0 &&
          *temp > 0) // Detects underflow of positive result with two negative
                     // arguments
      {
        throw std::underflow_error("Underflow detected, last good value: ");
      }
...
//  NOTE:
//    You will see the unary ('+') operator used in front of the variables in
//    the test_XXX methods. This forces the output to be a number for cases
//    where cout would assume it is a character.

template <typename T> void test_overflow() {
  //        The add_numbers template function will overflow in the second method
  //        call You need to change the add_numbers method to: ✔️1. Detect
  //        when an overflow will happen ✔️2. Prevent it from happening
  //        ✔️3. Return the correct value when no overflow happened or
  //        ✔️4. Return something to tell test_overflow the addition failed
  //        NOTE: The add_numbers method must remain a template in the
  //        NumericFunctions header.
  //
  ...
  //        ✔️3. A successful result displays the same result as before you
  //        changed the method NOTE: You cannot change anything between START /
  //        END DO NOT CHANGE
  //              The test_overflow method must remain a template in the
  //              NumericOverflows source file
  //
  //  There are more than one possible solution to this problem.
  //  The solution must work for all of the data types used to call
  //  test_overflow() in main().

  // START DO NOT CHANGE
  //  how many times will we iterate
  const unsigned long int steps = 5;
  // how much will we add each step (result should be: start + (increment *
  // steps))
...
template <typename T> void test_underflow() {
  //        The subtract_numbers template function will underflow in the second
  //        method call You need to change the subtract_numbers method to:
  ...
  //        ✔️4. Return something to tell test_underflow the subtraction
  //        failed NOTE: The subtract_numbers method must remain a template in
  //        the NumericFunctions header.
  //
  ...
  //        changed the method NOTE: You cannot change anything between START /
  //        END DO NOT CHANGE
  //              The test_underflow method must remain a template in the
  //              NumericOverflows source file
  //
  //  There are more than one possible solution to this problem.
  //  The solution must work for all of the data types used to call
  ...
  //  how many times will we iterate
  const unsigned long int steps = 5;
  // how much will we subtract each step (result should be: start - (increment *
  // steps))
...
void do_overflow_tests(const std::string &star_line) {
  ...
  // Testing C++ primative times see:
  // https://www.geeksforgeeks.org/c-data-types/ signed integers
  test_overflow<char>();
